...
$resultsPath = Join-Path $resultsDirectory "playmode-results.xml"
...
Write-Host "Running Unity $requiredUnityVersion Play Mode tests..."
...
if ($testRun.result -ne "Passed" -or $failed -ne 0 -or $total -eq 0) {
    Write-Error "Play Mode tests failed or no tests ran: $passed passed, $failed failed, $total total. See '$resultsPath' and '$logPath'." -ErrorAction Continue
    exit 1
}

Write-Host "Play Mode tests passed: $passed passed, $total total."
exit 0
