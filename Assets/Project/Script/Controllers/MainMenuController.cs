...
namespace Gazeus.DesafioMatch3.Controllers
{
    public sealed class MainMenuController : MonoBehaviour
    {
        ...
        private void PlayIntro()
        {
            ...
            _joker.localScale = Vector3.one * 0.7f;
            _joker.localRotation = Quaternion.Euler(0.0f, 0.0f, -10.0f);

            _title.localScale = Vector3.one * 0.9f;
            ...
            _playButtonTransform.localScale = Vector3.one * 0.9f;
            ...
            intro.Join(DOTween.To(() => _background.color, color => _background.color = color,
            ...
            for (int index = 0; index < _suits.Length; index++)
            {
                ...
                intro.Insert(delay, DOTween.To(() => suit.anchoredPosition,
                    position => suit.anchoredPosition = position, suitFinalPositions[index], 0.32f)
                    .SetEase(Ease.OutBack));
                intro.Insert(delay, suit.DOScale(Vector3.one, 0.32f).SetEase(Ease.OutBack));
                intro.Insert(delay, suit.DORotate(Vector3.zero, 0.32f, RotateMode.Fast).SetEase(Ease.OutQuad));
            }

            intro.Insert(0.3f, DOTween.To(() => _joker.anchoredPosition,
                position => _joker.anchoredPosition = position, jokerFinalPosition, 0.35f).SetEase(Ease.OutBack));
            intro.Insert(0.3f, _joker.DOScale(Vector3.one, 0.35f).SetEase(Ease.OutBack));
            intro.Insert(0.3f, _joker.DORotate(Vector3.zero, 0.35f, RotateMode.Fast).SetEase(Ease.OutQuad));
            intro.Insert(0.58f, _joker.DOPunchScale(Vector3.one * 0.1f, 0.16f, 4));

            intro.Insert(0.36f, _title.DOScale(Vector3.one, 0.24f).SetEase(Ease.OutQuad));
            intro.Insert(0.36f, Fade(_titleCanvasGroup, 1.0f, 0.24f));

            intro.Insert(0.6f, DOTween.To(() => _playButtonTransform.anchoredPosition,
                position => _playButtonTransform.anchoredPosition = position, playFinalPosition, 0.3f)
                .SetEase(Ease.OutBack));
            intro.Insert(0.6f, _playButtonTransform.DOScale(Vector3.one, 0.3f).SetEase(Ease.OutBack));
            intro.Insert(0.6f, Fade(_playButtonCanvasGroup, 1.0f, 0.2f));
            intro.OnComplete(() => BeginIdle(playFinalPosition, jokerFinalPosition));
        }
