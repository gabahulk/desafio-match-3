...
{
    internal static class PlayModeTestFixture
    {
        private static readonly int[] AvailableColors = { 0, 1, 2, 3 };

        internal static IEnumerator RunCreationScenario(
            Board board,
            Vector2Int from,
            Vector2Int to,
            ...
            int expectedTileId)
        {
            yield return RunGameplay(board, from, to,
                controller => Assert.That(CountSpecials(controller.Board), Is.Zero,
        ...
        internal static IEnumerator RunCombinationScenario(Board board, Action<Board> assertFinalBoard)
        {
            yield return RunSpecialScenario(
                board, "seeded special pair", assertFinalBoard);
        }

        internal static IEnumerator RunIndirectChainScenario(Board board, Action<Board> assertFinalBoard)
        {
            yield return RunSpecialScenario(
                board, "seeded indirect chain specials", assertFinalBoard);
        }

        private static IEnumerator RunSpecialScenario(
            Board board,
            string initialSpecialDescription,
            Action<Board> assertFinalBoard)
        {
            yield return RunGameplay(board, new Vector2Int(4, 4), new Vector2Int(5, 4),
                controller => Assert.That(CountSpecials(controller.Board), Is.EqualTo(2),
                    $"The scenario must begin with exactly the {initialSpecialDescription}."),
                controller => assertFinalBoard(controller.Board));
        }

        internal static IEnumerator RunGameplay(
            Board board,
            Vector2Int from,
            Vector2Int to,
        ...
        internal static void AssertIdsAbsent(Board board, params int[] tileIds)
        {
        ...
        internal static void AssertIdSurvived(Board board, int tileId)
        {
            Assert.That(ContainsTileId(board, tileId), Is.True,
                $"Expected unaffected tile {tileId} to survive.");
        }

        internal static bool ContainsTileId(Board board, int tileId)
        {
        ...
        internal static Board CreateBoard(params string[] rows)
        {
            ...
            for (int y = 0; y < rows.Length; y++)
            {
                if (rows[y] == null || rows[y].Length != width)
                {
                    throw new ArgumentException(
                        $"Row {y} must contain exactly {width} symbols.", nameof(rows));
                }
        ...
        internal static Board CreateCombinationBoard(SpecialType first, SpecialType second)
        {
            Board board = CreateBoard(
                "RGBYRGBYR", "GBYRGBYRG", "BYRGBYRGB",
                "YRGBYRGBY", "RGBYRGBYR", "GBYRGBYRG",
                "BYRGBYRGB", "YRGBYRGBY", "RGBYRGBYR");
        ...
        internal static Board CreateColorCombinationBoard(
            SpecialType first,
            SpecialType second,
            bool useCornerTargets = false)
        {
            ...
            Vector2Int[] targets = useCornerTargets
                ...
                : new[] { new Vector2Int(1, 1), new Vector2Int(7, 1), new Vector2Int(1, 7) };
        ...
        internal static Board CreateIndirectChainBoard(
            SpecialType source,
            SpecialType target,
        ...
        internal static int[] GetTileIdsWithColor(Board board, int color)
        {
        ...
        private static void AssertRenderedBoardMatchesDomain(Board board)
        {
            ...
            for (int x = 0; x < board.Width; x++)
            {
                ...
                Assert.That(tileSpot.transform.childCount, Is.EqualTo(tile.IsEmpty ? 0 : 1),
        ...
        private static void AssertBoardHasNoMatches(Board board)
        {
            ...
            for (int x = 0; x < board.Width; x++)
            {
                if (x >= 2)
                {
                    Assert.That(board[x, y].Color == board[x - 1, y].Color &&
                                board[x - 1, y].Color == board[x - 2, y].Color,
                        Is.False, $"Scenario begins with a horizontal match ending at ({x}, {y}).");
                }

                if (y >= 2)
                {
                    Assert.That(board[x, y].Color == board[x, y - 1].Color &&
                                board[x, y - 1].Color == board[x, y - 2].Color,
                        Is.False, $"Scenario begins with a vertical match ending at ({x}, {y}).");
                }
            }
        }
